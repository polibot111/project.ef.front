    ...
    >
      <template v-slot:top>
        <v-toolbar flat color="white">
          ...
          <v-spacer />
          <v-btn color="primary" variant="flat" @click="handleCreateStaff"
            >Yeni Ekle</v-btn
          >
        </v-toolbar>
      </template>

      <template v-slot:item.statu="{ item }: any">
        <div class="d-flex align-center">
          <v-icon
            :color="item.statu ? 'success' : 'error'"
            class="me-2"
            size="small"
            >{{ item.statu ? "mdi-check" : "mdi-close" }}</v-icon
          >
          <span>{{ item.statu ? "Aktif" : "Pasif" }}</span>
        </div>
      </template>
      <template v-slot:item.fabricColorName="{ item }: any">
        <div class="d-flex align-center">
          <div
            class="color-square"
            :style="{ backgroundColor: item.fabricType.color.rgbCode }"
          ></div>
          <span class="ms-2">{{ item.fabricType.color.name }}</span>
        </div>
      </template>
          ...
          <v-form ref="staffForm" @submit.prevent>
            <v-row>
              ...
              <v-col cols="12" sm="12" class="d-flex align-center">
                <span class="mr-3">Durumu: </span>
                <v-switch
                  color="green"
                  base-color="red"
                  v-model="model.statu"
                  :label="model.statu ? 'Aktif' : 'Pasif'"
                  dense
                  hide-details
                ></v-switch>
              </v-col>
...
const headers: any = ref([
  { title: "Personel Adı", value: "name", align: "start" },
  { title: "Departmanı", value: "departmentName", align: "start" },
  { title: "Telefon", value: "telephone", align: "start" },
  { title: "Adres", value: "adress", align: "start", sortable: true },
  { title: "Durumu", value: "statu", align: "start", sortable: true },
...
const confirm = ref({
  ...
  handlerShow: (item: any, type?: string, otherItem?: any) => {
    ...
    confirm.value.title = "Personel Sil!";
    confirm.value.body = `
            <p>
              <strong>${item.name}</strong> isimli personelyi silmek istediğinize emin misiniz?
            </p>
          `;
  },
});
...
const handleFilter = async () => {
  ...
  await fetchStaffList(modelpool)
...
const handleCreateStaff = () => {
  ...
  dialog.value.title = "Personel Oluştur";
...
const handleSubmit = async () => {
  ...
  if (staffForm.value?.isValid) {
    ...
    const url = dialog.value.type === "new" ? "/OrderOwner" : "/OrderOwner";
    ...
    dialog.value.loading = false;
    if (!data.value.succeeded) {
      toast.error(data.value.message || "Personel eklenirken hata oluştu");
    } else {
...
const editItem = (item: any) => {
  ...
  dialog.value.show = true;
  dialog.value.title = "Personeli Düzenle";
...
const deleteItem = async (item: any) => {
  ...
  if (!data.value?.succeeded) {
    loading.value.table = false;
    toast.error(data.value.message || "Personel silinirken hata oluştu");
    return false;
  } else {
