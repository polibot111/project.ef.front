<template>
  <div class="page-storage_management">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat color="white">
          ...
          <v-spacer />
          <v-btn color="primary" variant="flat" @click="handleCreateStorage"
            >Yeni Ekle</v-btn
          >
        </v-toolbar>
      </template>
...
const headers: any = ref([
  { title: "Depo Adı", value: "name", align: "start" },
...
const confirm = ref({
  ...
  handlerShow: (item: any, type?: string, otherItem?: any) => {
    ...
    confirm.value.body = `
          <p>
            <strong>${item.name}</strong> isimli Depoyu silmek istediğinize emin misiniz?
          </p>
        `;
  },
});
...
const handleCreateStorage = () => {
  ...
  dialog.value.title = "Depo Oluştur";
...
const handleSubmit = async () => {
  ...
  if (customerForm.value?.isValid) {
    ...
    dialog.value.loading = false;
    if (!data.value.succeeded) {
      toast.error(data.value.message || "Depo eklenirken hata oluştu");
    } else {
...
const editItem = (item: any) => {
  ...
  dialog.value.show = true;
  dialog.value.title = "Depoyu Düzenle";
...
const deleteItem = async (item: any) => {
  ...
  const url = "/Storage/DeleteStorage";
